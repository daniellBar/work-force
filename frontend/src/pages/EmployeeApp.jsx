...
import {
  loadEmployees,
  removeEmployee,
} from "../store/action/employeeActions.js";
...
class _EmployeeApp extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.filterBy.name !== this.props.filterBy.name ||
      prevProps.filterBy.group !== this.props.filterBy.group
    ) {
  ...
  onDelete = (employeeId) => {
    ...
    loggedInUser && loggedInUser.isAdmin
      ? this.handleRemoveEmployee(employeeId)
      : this.openSnackbar();
  };
  ...
  setSelectedEmployee = (employee) => {
    if (employee) {
      ...
      const currIndex = employees.findIndex(
        (employee) => employee._id === selectedEmployee._id
      );
      selectedEmployee.nextEmployee = employees[currIndex + 1] || employees[0];
      selectedEmployee.prevEmployee =
        employees[currIndex - 1] || employees[employees.length - 1];
    ...
    return (
      <section className="employee-app main-container">
        <div>
          ...
          <EmployeeNavBar
            buildFilterBy={this.buildFilterBy}
          ...
          <div className="employee-secondary-container flex">
            ...
            <div className="employee-third-container">
              ...
              <EmployeeDropdownsRow
                buildFilterBy={this.buildFilterBy}
                setSort={this.setSort}
              />
              <EmployeeList
                employees={this.paginateEmployeesForDisplay(
                  employees,
                  currPage,
                  employeesPerPage
                )}
        ...
        <Modal isOpen={this.state.isModalOpen}>
          <div className="modal-header">
            <h1 className="modal-title">{`${
              this.state.selectedEmployee ? "Edit" : "Add"
            } Employee`}</h1>
          </div>
          <EmployeeEdit
            employee={employee}
...
export const EmployeeApp = connect(
  mapStateToProps,
  mapDispatchToProps
)(_EmployeeApp);
