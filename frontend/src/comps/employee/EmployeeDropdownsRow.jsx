...
class _EmployeeDropdownsRow extends Component {
    ...
    handleChange = (e) => {
        ...
        field === 'activeGroupFilter' ? this.handleFilterChange(value) : this.handleSortChange(value)
    }

    handleSortChange = (by) => {
        this.setState({ activeSort: by }, () => {
            this.props.setSort({ by: this.state.activeSort, direction: this.state.sortDirection })
        })
    }
    ...
    render() {
        ...
        const groupsMap =
            [
                { name: 'All Employees', filterBy: 'all' }, { name: 'Wifi', filterBy: 'wifi' },
                { name: 'Web', filterBy: 'web' }, { name: 'Algorithms', filterBy: 'algorithms' },
                { name: 'Automation Infra', filterBy: 'automation infra' }, { name: 'Business', filterBy: 'business' },
        ...
        return (
            <section className="Employee-dropdowns-row">
                <div className="filter-select-container">
                    ...
                    >
                        {groupsMap.map((group, idx) => <MenuItem value={group.filterBy} key={idx}>{group.name}</MenuItem>)}
                    </Select>
                </div>
                <div className="sort-select-container">
                    ...
                    <div className="sort-btns flex">
                        <ArrowUpwardIcon onClick={() => this.onClickSortButtons('up')} fontSize='small' htmlColor={sortDirection === 'up' ? '#0ec9ae' : ''} />
                        <ArrowDownwardIcon onClick={() => this.onClickSortButtons('down')} fontSize='small' htmlColor={sortDirection === 'down' ? '#0ec9ae' : ''} />
                    </div>
                </div>
            </section>
        )
...
export const EmployeeDropdownsRow = connect(mapStateToProps, mapDispatchToProps)(withRouter(_EmployeeDropdownsRow))